...
pub fn unwrap(ke: &KeyEnvelope) -> Result<AES128Key> {
    ...
    for kek in &conf.keks {
        if kek.label == ke.kek_label {
            let key = ke.unwrap(&kek.kek.to_be_bytes())?;
            return Ok(AES128Key::from_be_bytes(key));
        }
    }
...
pub fn wrap(label: &str, key: AES128Key) -> Result<KeyEnvelope> {
    if label.is_empty() {
        return KeyEnvelope::new("", None, &key.to_be_bytes());
    }

    let conf = config::get();
    for kek in &conf.keks {
        if kek.label == *label {
            return KeyEnvelope::new(label, Some(&kek.kek.to_be_bytes()), &key.to_be_bytes());
        }
    }
